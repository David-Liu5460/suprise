...
function explode(x, y, hue) {
  ...
  if (showText) {
    ...
    for (let i = 0; i < data.pts.length; i++) {
      ...
      particles.push({ x, y, vx: Math.cos(rand(0, Math.PI * 2)) * spd, vy: Math.sin(rand(0, Math.PI * 2)) * spd, alpha: 1, fade: rand(0.0015, 0.004), stayUntil: performance.now() + rand(15000, 20000), size, hue, sat, light, type: 'text', tx: bx + tp.x, ty: by + tp.y, arrived: false })
      if (Math.random() < 0.35) particles.push({ x, y, vx: 0, vy: 0, alpha: 1, fade: rand(0.0015, 0.004), stayUntil: performance.now() + rand(15000, 20000), size, hue, sat, light, type: 'text', tx: bx + tp.x + rand(-1, 1), ty: by + tp.y + rand(-1, 1), arrived: false })
    }
    const extra = 30
